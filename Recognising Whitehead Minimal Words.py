...
for u in U(2,2): 
    print(words_getting_smaller(u,2))
...
f_6_train_arrays_2_1=[f_6(t[0],2) for t in training_data_2_1 ] # Making the f_6 matrix 

f_6_train_arrays_non_min_2_1=f_6_train_arrays_2_1[:5000] # Min and non-min arrays
f_6_train_arrays_min_2_1=f_6_train_arrays_2_1[5000:]

V6_train_2_1=np.row_stack(f_6_train_arrays_2_1)  

V6_non_min_train_2_1=np.row_stack(f_6_train_arrays_non_min_2_1) 
V6_min_train_2_1=np.row_stack(f_6_train_arrays_min_2_1) 
...
def perform_linear_regression(X_train, y_train, label):
    ...
    # Coefficients
    coefficients_B = model.coef_
    intercept = model.intercept_

    # Assess the model
    r_squared = model.score(X_train, P)

    # Return results
    return coefficients_B, intercept, r_squared
...
datasets = [
    V0_train_2_1, V0_train_2_2, V0_train_2_3, V0_train_2_4, V0_train_2_5,
    V2_train_2_1, V2_train_2_2, V2_train_2_3, V2_train_2_4, V2_train_2_5,
    V3_train_2_1, V3_train_2_2, V3_train_2_3, V3_train_2_4, V3_train_2_5,
    V4_train_2_1, V4_train_2_2, V4_train_2_3, V4_train_2_4, V4_train_2_5,
    V5_train_2_1, V5_train_2_2, V5_train_2_3, V5_train_2_4, V5_train_2_5,
    V6_train_2_1, V6_train_2_2, V6_train_2_3, V6_train_2_4, V6_train_2_5
]

block_r_squared_values = {}

for i, dataset in enumerate(datasets, start=1):
    block_number = (i - 1) // 5 + 1
    run_number = (i - 1) % 5 + 1
    label = f"{block_number}_{run_number}"

    coefficients, intercept, r_squared = perform_linear_regression(dataset, training_data_2_1_labels, label)
    
    print(f"Block {block_number}, Run {run_number}")
    print(f"Coefficients: {coefficients}")
    print(f"Intercept: {intercept}")
    print(f"R-squared: {r_squared}")
    print("="*30)

    # Update block's R-squared values
    block_r_squared_values.setdefault(block_number, []).append(r_squared)

# Calculate and print average R-squared for each block
print("Average R-squared values:")
for block_number, r_squared_values in block_r_squared_values.items():
    average_r_squared = np.mean(r_squared_values)
    print(f"Block {block_number}: {average_r_squared}")
